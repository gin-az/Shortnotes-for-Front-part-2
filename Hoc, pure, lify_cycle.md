...
Для функциональных компонентов можно исп-ть `React.Memo` - это `HOC` (функция высшего порядка). Он принимает компонент и возвращает компонент, __по-умолчанию сравнение происходит поверхностное__. По мимо компонента, вторым аргументом может принимать функцию сравнения (кастомную).
